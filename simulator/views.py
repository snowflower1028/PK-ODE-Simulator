...
#
# 처음 20,000 으로 정한 이유는 NCA 의 종말상 기울기 탐색이 점 수의 제곱으로
# 늘었기 때문이다(10,000점 1.3초, 30,000점 8.0초, 60,000점 44.9초). 그 탐색은
# 이제 O(n) 이라(nca._best_window_incremental) 더는 이유가 아니다 — 경구 1구획
# 7회 투여 /simulate/ 가 20,000점 0.36초, 50,000점 0.89초, 100,000점 1.83초.
#
# 그래도 올리지 않은 것은 /sweep/ 이 같은 상한을 쓰기 때문이다. 스윕은 한
# 요청에 최대 40번 다시 풀므로, 상한을 2.5배 올리면 가장 큰 스윕도 2.5배
# 무거워진다(워커 2개 · 512MB). 올리려면 스윕 쪽 상한을 따로 두고 올릴 것.
_MAX_T_STEPS = 20_000
